...
const ItemDataGrid = React.forwardRef(
  (
    {
      rows = [],
      columns = [],
    ...
    const commitEdit = async () => {
      ...
      const processedRow = processRowUpdate
        ? await Promise.resolve(processRowUpdate(newRow, oldRow))
        : newRow;
      if (onRowUpdate) {
    ...
    return (
      ...
      <div
        ref={ref}
        style={{ height: '100%', width: '100%', overflow: 'auto', border: '1px solid black' }}
      >
            ...
            {sortedRows.map((row, rowIndex) => {
              ...
              return (
                <tr
                  ...
                  style={{
                    height: `${rowHeight}px`,
                    backgroundColor: selectedCoords.rowIndex === rowIndex ? '#e0e0e0' : undefined,
                  }}
                    ...
                    return (
                      ...
                      >
                        {isEditing ? (
                          ...
                          col.renderCell({
                            api: { getSortedRowIds: () => sortedRows.map(getRowId) },
                            id: rowId,
                            row,
                          })
                        ) : (
