...
// 1. 라이브러리 대신 직접 만든 PrintButton을 임포트합니다.
...
const Setup = () => {
  ...
  const generateMaterialCode = (materialType, thickness) => {
    ...
    const mType = (fullType[0]?.match(/[a-zA-Z]+/g) || ['']).join('');
    ...
    if (fullType.length > 2) {
      ...
      let w_Integer = ((fullType[1] || '0').split('.')[0] || '0').padStart(3, '0');
      let w_Float = fullType[1]?.split('.')[1] || '0';
  ...
  const handleInputChange = (field, value) => {
    const updatedValue = typeof value === 'string' ? value.toUpperCase().replace(/X/g, '*') : value;
    const updatedRow = { ...currentRow, [field]: updatedValue };

    if (currentTable === 'left') {
      if (field === 'materialType' || field === 'length') {
        updatedRow.materialCode = generateMaterialCode(updatedRow.materialType, updatedRow.length);
      }
    } else {
      if (field === 'bbCode') {
        updatedRow.systemCode = generateSystemCode(updatedRow.bbCode);
      }
    }
  ...
  const handleKeyDown = (e, index) => {
    if (e.key === 'Enter') {
      if (inputRefs.current[index] && index < inputRefs.current.length - 1) {
